class EventCalendar {
  ...
  static getFormatTime(mins) {
    ...
    let minutes = mins % 60;
    return `${EventCalendar.addZeroForTime(
      hours
    )}:${EventCalendar.addZeroForTime(minutes)}`;
  }
  ...
  static setNotificationTimeout(startTime, title) {
    const date = new Date();
    startTime += 480;
    let currentTime = EventCalendar.parseFormatTime(
      `${EventCalendar.addZeroForTime(
        date.getHours()
      )}:${EventCalendar.addZeroForTime(date.getMinutes())}`
    );
    currentTime += 480;
  ...
  deleteEvent(id) {
    EventCalendar.tasks = EventCalendar.tasks.filter(
      (el) => el.id !== Number(id)
    );
    this.renderEvents();
  }
  ...
  renderEvents() {
    const timePoints = Array.from(
      document.querySelector("#timeTable").childNodes
    );
    ...
    if (EventCalendar.tasks.length) {
      EventCalendar.tasks.map((el) => {
        const timePointIndex = timePoints.findIndex(
          (item) => EventCalendar.convertTimePoint(item) === el.start
        );
        ...
        if (timePointIndex === el.start) {
          ...
          td.setAttribute(
            "style",
            `background-color: ${EventCalendar.hexToRGB(
              el.color,
              0.5
            )}; border-left: 3px solid ${el.color}`
          );
